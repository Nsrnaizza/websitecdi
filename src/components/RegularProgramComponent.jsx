...
import { getRegularPrograms } from "../api/regulerApi";
  ...
  useEffect(() => {
    const fetchPrograms = async () => {
      try {
        ...
        console.log("Fetched Programs:", data); // Log data yang diterima
        setPrograms(data);
        setError(null);
      } catch (err) {
...
// In RegularProgramComponent.jsx
const containerRef = useRef(null);
const { scrollYProgress } = useScroll({
  target: containerRef,
  offset: ["start start", "end start"],
  layoutEffect: false // Add this to fix the warning
});
  ...
  if (programs.length === 0 && !loading) {
    return (
      <div className="d-flex justify-content-center align-items-center" style={{ height: "100vh" }}>
        <div className="alert alert-info">Tidak ada program yang tersedia saat ini.</div>
      </div>
    );
  }

  return (
    <div className="regular-program" ref={containerRef}>
      {/* Animated Background */}
      <motion.div
          ...
          <div className="row align-items-center">
            <div className="col-lg-6 order-2 order-lg-1 text-center text-lg-start">
              ...
              <h4 className="fw-normal">
                Daftar program pelatihan <span className="text-lowercase">Reguler</span>
                <br />
        ...
        <Container>
          <Row className="justify-content-center g-4">
            {programs.map((program, index) => {
              console.log("Rendering Program:", program); // Log setiap program yang dirender
              return (
                <Col xs={12} sm={6} md={6} lg={3} key={program.id || index}>
                  <Fade bottom delay={index * 50} triggerOnce>
                    ...
                    >
                      <a href={`reguler/${program.slug || program.id}`} className="w-100 text-decoration-none h-100 d-block">
                        <div
...
<h5
  ...
  style={{
    ...
    letterSpacing: "0.5px",
    minHeight: "1.8rem",
    maxWidth: "100%", // Tambahkan ini untuk memastikan truncate bekerja
    whiteSpace: "nowrap",
    overflow: "hidden",
